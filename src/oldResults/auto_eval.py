...
if __name__ == "__main__":  
    ...
    for config_file in files:
        ...
        for i, x in enumerate(list_benchmark_files):
            ...
            # find best known in csv file
            bks = open("RCPSP_BKS.csv", "r")
            lines = bks.readlines()
            for line in lines: 
                nameCSV = line.split(",")[0]                            
                if nameCSV == instanceName:
                    bestKnown = line.split(",")[1].split("\n")[0]
                    break
            ...
            for j, y in enumerate(list_program_calls):
                ...
                percentage = round((int(bestKnown)/avg_makespan)*100, 3)
                
                f.write(str(avg_makespan) + "\t" + str(bestKnown) + "\t" + str(percentage) +"\n")   
